...
public class tk2dButton : MonoBehaviour 
{
	...
	{
		/*if (audio && source && PlayerInfo.instance.isSound)
		{*/
	...
	void Update ()
	{
		if (!buttonDown && Input.GetMouseButtonDown(0))
        {
            ...
            if (collider.Raycast(ray, out hitInfo, 1.0e8f))
            {
				...
				{
					//UiModel.instance.buttonPress = true;
//					Debug.Log(UiModel.instance.buttonPress);
					
					Invoke("buttonUnpress",0.3f);
	...
	void OnDestroy() 
	{
		//UiModel.instance.buttonPress = false;
